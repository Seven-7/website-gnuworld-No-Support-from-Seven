			...
			if (check_secure_form("do_close" . $_GET["ID"] . $_POST["from_email"])) {
				...
				if ($o1 = pg_fetch_object($r1)) {
					...
					if ($updated) {
						...
						mail($_POST["from_email"],"[" . NETWORK_NAME . " CService Complaints] " . $_GET["ID"] . " - Closed by user",$mm,"From: " . NETWORK_NAME . " Channel Service <" . OBJECT_EMAIL . ">\nReply-to: DO.NOT@REPLY.THANKS\nX-Mailer: " . NETWORK_NAME . " CService Complaint Module\n\n");
...
switch ($_GET["A"]) {
	...
	case 'resolve':
		...
		if ($updated) {
			...
			$mm .= "Thanks for using our Complaint System.\n\n";
			mail($daobj->from_email,"[" . NETWORK_NAME . " CService Complaints] " . $_GET["ID"] . " - Resolved / Closed",$mm,"From: " . NETWORK_NAME . " Channel Service <" . OBJECT_EMAIL . ">\nReply-to: DO.NOT@REPLY.THANKS\nX-Mailer: " . NETWORK_NAME . " CService Complaint Module\n\n");
