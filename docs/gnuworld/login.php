...
if ($username != "" && $password != "") {
	...
	*/
		...
		$expire=time()+COOKIE_EXPIRE; // Login expiration.
		...
		$ENABLE_COOKIE_TABLE = 1;
		pg_safe_exec("INSERT INTO webcookies (user_id,cookie,expire,is_admin,tz_setting) VALUES (" . (int)$user_id . ",'" . $cookie . "'," . (int)$expire . "," . (int)$is_admin . ",'" . $tz_setting . "')");
		$ENABLE_COOKIE_TABLE = 0;
		$dynts = time();
		$cook2 = md5( $dynts . CRC_SALT_EXT1 . $cookie );
		if (COOKIE_DOMAIN!="") {
			SetCookie("auth",$username . ":" . (int)$user_id . ":" . (int)$time . ":" . $cookie . ":" . (int)$dynts . ":" . $cook2,time()+COOKIE_EXPIRE,"/",COOKIE_DOMAIN);
