...
/* $Id: passwd.php,v 1.6 2006/07/17 00:35:55 nighty Exp $ */
std_connect();
				...
				if (strtolower($dauser->verificationdata) == strtolower($pass1)) {
					...
					if (strtolower($dauser->email) == strtolower($pass1)) {
						...
						if (chk_password($dauser->user_name,$pass0,-1)<=0) {
							$da_error = 6; // possible bruteforce attack (but rare, user must be already logged in), will add security later...
						} else {
							// change password
...
if (BOFH_PASS_USER || ($admin>0 && (BOFH_PASS_USER || BOFH_PASS_ADMIN))) {
	echo "<b>OR</b> at least 20 chars including at least one space. (passphrase).<br><br>\n";
}
echo "Current Password :\t<input type=password name=pass0 maxlength=255 size=30> (for verification)<br><br>\n";
