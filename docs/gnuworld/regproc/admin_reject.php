...
//	echo "Put <select name=noregtype><option value=0 selected>Both username and channel</option><option value=1>Only username</option><option value=2>Only channel</option></select>&nbsp;in NOREG.<br>\n";
	...
	if ($$tmp==1) {
		echo "<tr><td align=right><b>type</b>&nbsp;</td><td>[<input onclick=\"fill('NR_reason',0)\" type=radio name=NR_type value=2>Abuse]&nbsp;[<input onclick=\"fill('NR_reason',1)\" type=radio checked name=NR_type value=4>Fraud Username]</td></tr>\n";
		echo "<tr><td align=right><b>never_reg</b>&nbsp;</td><td><input type=checkbox name=NR_never_reg value=1 checked></td></tr>\n";
	} else {
	...
	echo "<select name=NR_expire_period0><option selected value=30>30</option><option value=60>60</option><option value=90>90</option></select>";
	...
	if ($$tmp==1) {
		echo "<tr><td align=right><b>reason</b>&nbsp;</td><td><input type=text name=NR_reason size=35 value=\"bogus username (" . str_replace("\"","&quot;",$c_name) . ")\"></td></tr>\n";
	} else {
		...
		for ($x=0;$x<pg_numrows($uuu);$x++) {
			...
			if (pg_numrows($ttmp)==0) {
				...
				if ($$tmp==1) {
					...
					echo "<td>[<input onclick=\"fill('supreason$x',0)\" type=radio name=suptype$x value=1>Abuse]<br>[<input onclick=\"fill('supreason$x',1)\" checked type=radio name=suptype$x value=2>Fraud Username]</td>";
				//	echo "<td><input type=text name=supdays$x size=4 maxlength=3 value=" . $def_day_duration . "></td>";
					echo "<td><select name=supdays" . $x . "><option selected value=30>30</option><option value=60>60</option><option value=90>90</option></select></td>";
					echo "<td><input type=text name=supreason$x size=20 maxlength=80 value=\"bogus username (" . str_replace("\"","&quot;",$c_name) . ")\"></td>";
				} else {
				...
				//	echo "<td><input type=text name=supdays$x size=4 maxlength=3 value=" . $def_day_duration . "></td>";
					echo "<td><select name=supdays" . $x . "><option selected value=30>30</option><option value=60>60</option><option value=90>90</option></select></td>";
					echo "<td><input type=text name=supreason$x size=20 maxlength=80 value=\"registration fraud (" . str_replace("\"","&quot;",$c_name) . ")\"></td>";
				}
			} else {
