<?
/* $Id: toast_this.php,v 1.15 2009/06/04 19:18:28 mrbean_ Exp $ */

	define(TOASTER_FROM_MAIL,"toaster@yoursite.com");
...
//if ($admin==0) { local_seclog("Toast! mode=[" . $_POST["dmode"] . "], st=[" . $_POST["dstatus"] . "], sp=[" . $_POST["dsp"] . "], or=[" . $_POST["dorder"] . "], nb=[" . $_POST["dnb"] . "], cname=[" . $_POST["ccname"] . "]."); }
...
for ($x=0;$x<$gcount;$x++) {
	...
	if ($$delnoreg==1) {
		...
		$nr_q .= "now()::abstime::int4,'" . $adm_user . " (Toaster)','" . $dreason . "')";
		...
		if ($$fraud==1 && $notalready) {
			...
			$daq = "INSERT INTO noreg (user_name,email,channel_name,type,never_reg,for_review,expire_time,created_ts,set_by,reason) VALUES ('" . $username[$x] . "','" . $email[$x] . "','',4,1,0,0,now()::abstime::int4,'" . $adm_user . "','" . $freason . "')";
...
if (!$debug_me && $send_mail && $mail_lines>0) { //send the mail !!
	mail($_POST["sendlist"],"[TOASTER] " . $adm_user . " : " . $mail_lines . " username(s) suspended",$mmsg,
